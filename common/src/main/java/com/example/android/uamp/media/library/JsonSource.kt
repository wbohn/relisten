...
//    }
    ...
    @Throws(IOException::class)
    private fun downloadJson(catalogUri: Uri): List<JsonArtist> {
        val catalogConn = URL(catalogUri.toString())
        val reader = BufferedReader(InputStreamReader(catalogConn.openStream()))

        val listType = object : TypeToken<List<JsonArtist>>() {}.type
        //return Gson().fromJson<List<JsonArtist>>(reader, ArrayList<JsonArtist>::class.java)
        return Gson().fromJson<List<JsonArtist>>(reader, listType)
    }
}
...
fun MediaMetadataCompat.Builder.from(jsonArtist: JsonArtist): MediaMetadataCompat.Builder {
    // The duration from the JSON is given in seconds, but the rest of the code works in
    // milliseconds. Here's where we convert to the proper units.
    val durationMs = TimeUnit.SECONDS.toMillis(jsonArtist.duration)

    id = jsonArtist.id
    title = jsonArtist.name

    downloadStatus = STATUS_NOT_DOWNLOADED
    flag = MediaItem.FLAG_BROWSABLE

    artist = jsonArtist.artist
    album = jsonArtist.album
    duration = durationMs
    genre = jsonArtist.genre
    mediaUri = jsonArtist.source
    albumArtUri = jsonArtist.image
    trackNumber = jsonArtist.trackNumber
    trackCount = jsonArtist.totalTrackCount
    flag = MediaItem.FLAG_PLAYABLE

    // To make things easier for *displaying* these, set the display properties as well.
    displayTitle = jsonArtist.name
    displaySubtitle = jsonArtist.name
    displayDescription = jsonArtist.name
    displayIconUri = jsonArtist.name
...
class JsonArtists {
    var artists: List<JsonArtist> = ArrayList()
}
